...
class TestMessageAPI(MessageFixture):
    ...
    def test_success_search_message_for_conversation_1(
        self,
        client,
        conversation_1,
        message_1_user_text,
        message_2_bot_text,
        message_3_user_audio,
        message_4_user_image,
    ):
        # Arrange
        # Act
        response = client.get(
        ...
        assert data['total'] == 3
        assert {message['id'] for message in data['data']} == {
            message_1_user_text.id,
            message_2_bot_text.id,
            message_3_user_audio.id,
        }
